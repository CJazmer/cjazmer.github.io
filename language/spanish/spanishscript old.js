let Verbs = [ // Verb database ----------------------------------
    ...
    let SpanishVerbEndings = [ // all spanish verb endings ----------------------------------
    ...
    // Irregular Stuff ---------------------------------------------------------
    let verbsEnglishtoSpanishirregulars = [ // Irregular past tense database ----------------------------------
        ["ate", "comer"],
        ["said", "decir"],
        ["made", "hacer"],
        ["went", "ir"],
    ...
    let nounsEnglishtoSpanishirregulargender = [ // Irregular gendered nouns database ----------------------------------
        ["agua", "male"],
        ["mano", "female"],
        ["luz", "female"],
    ]
...
function Translate() {
    var spanishtoenglish = true;
    var righttextarea = document.getElementById("righttextarea");
    righttextarea.textContent = ""; // resets bottom textbox
    if (document.getElementById("langtolang").textContent == "Spanish to English") { spanishtoenglish = true }      // Left Language Spanish & Right Language English -> Left Language English & Right Language Spanish
    else if (document.getElementById("langtolang").textContent == "English to Spanish") { spanishtoenglish = false }      // Left Language Spanish & Right Language English -> Left Language English & Right Language Spanish




    // SPANISH TO ENGLISH -------------------------------------------------------------------------------------------------------------------------------------------------
    if (spanishtoenglish == true) {// If Spanish to english  (dont need to know gender of noun,) ---------------------------------------------------------------------
        ...
        for (var j = 0, len = ogparagraphwords.length; j < len; j++) { paragraphwords.push({ spanish: ogparagraphwords[j], english: "", partofspeech: "other" }) }; // fills new array based on old one
        ...
        //for (var i = 0; i < paragraphwords.length - 1; i++) { // flip adjective & Nounff
        //    if (paragraphwords[i-1].partofspeech == "adjective") {}
        //}
    ...
    // ENGLISH TO SPANISH---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    if (spanishtoenglish == false) { // If English to Spanish  (need to know gender of spanish noun,) -----------------------------------------------------------------------------------------------------
...
function Flip() { // Flip Languages ----------------------------------------
    ...
    if (langtolang.textContent == "Spanish to English") { langtolang.textContent = "English to Spanish"; spanishtoenglish = false;} // flip to English to Spanish
    else if (langtolang.textContent == "English to Spanish") { langtolang.textContent = "Spanish to English"; spanishtoenglish = true;}  // flip to Spanish to English
}
...
function updateword(arrayitem, english, partofspeech) {
    arrayitem.english = english; arrayitem.partofspeech = partofspeech;
}
...
function TranslateWord(table, word, partofspeech) {
    if (word.english == "") { table.forEach(tableword => { 
        if (word.spanish == tableword.spanish) word.english = tableword.english; word.partofspeech = partofspeech; }) }
}
